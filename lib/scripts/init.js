...
function createPage(res) {
  ...
  if (isReact) {
    const indexPagePath2 = path.resolve(process.cwd(), `src/pages/index/${(!isTs ? "App.js" : "App.tsx")}`);
    ...
    const indexPagePath = path.resolve(process.cwd(), `src/pages/index/index.${folder}`);
    ...
    const indexPagePath = path.resolve(process.cwd(), `src/pages/index/${(!isTs ? "index.js" : "index.ts")}`);
...
function prepare(projectName, options, developOptions) {
  ...
  require("../utils/install")(printSuccess);
}
